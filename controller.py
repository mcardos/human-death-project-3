...
class HealthfyController:
    """
    Controller class of the game that translates input from the user into
    arguements that can be used to play the game.

    Attributes:
        model = An instance of the model class that references the features
        of the game model.
    """
    ...
    def get_input(self):
        """
        Gets the input from the player and translates it into one
        of the 5 commands.

        Returns:
            A string with the current status of the humanoid
            and an updated healthbar.
        """
        player_input = input(
            "Press a button to command the humanoid!\
                (or enter h for help, q to quit): ")
    ...
    def _help(self):
        ...
        """
        print("Click one of the buttoms on the screen to make the Humanoid\
            take action")
